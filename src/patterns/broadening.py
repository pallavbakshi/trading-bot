import numpy as np
import pandas as pd


def _fit_trendline(points):
    if len(points) < 2:
        return None, None, 0
    x = np.array([p["bar_index"] for p in points], dtype=float)
    y = np.array([p["price"] for p in points], dtype=float)
    coeffs = np.polyfit(x, y, 1)
    slope, intercept = coeffs
    y_pred = np.polyval(coeffs, x)
    ss_res = np.sum((y - y_pred) ** 2)
    ss_tot = np.sum((y - np.mean(y)) ** 2)
    r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0
    return slope, intercept, r_squared


def detect_broadening(df: pd.DataFrame, pivots: pd.DataFrame,
                      min_touches: int = 4,
                      min_pattern_bars: int = 15,
                      max_pattern_bars: int = 200) -> list[dict]:
    """Detect broadening formations (megaphone / expanding triangle).

    Opposite of a triangle: upper trendline slopes UP, lower trendline slopes DOWN.
    Indicates increasing volatility and indecision.
    """
    peaks = pivots[pivots["type"] == "peak"].to_dict("records")
    troughs = pivots[pivots["type"] == "trough"].to_dict("records")

    if len(peaks) < 2 or len(troughs) < 2:
        return []

    patterns = []

    for start_idx in range(len(peaks) - 1):
        for end_idx in range(start_idx + 1, min(start_idx + 6, len(peaks))):
            window_peaks = peaks[start_idx:end_idx + 1]

            bar_start = window_peaks[0]["bar_index"]
            bar_end = window_peaks[-1]["bar_index"]
            span = bar_end - bar_start

            if span < min_pattern_bars or span > max_pattern_bars:
                continue

            window_troughs = [t for t in troughs if bar_start <= t["bar_index"] <= bar_end]
            total_touches = len(window_peaks) + len(window_troughs)
            if total_touches < min_touches or len(window_troughs) < 2:
                continue

            upper_slope, upper_int, upper_r2 = _fit_trendline(window_peaks)
            lower_slope, lower_int, lower_r2 = _fit_trendline(window_troughs)

            if upper_slope is None or lower_slope is None:
                continue
            if upper_r2 < 0.6 or lower_r2 < 0.6:
                continue

            avg_price = (np.mean([p["price"] for p in window_peaks]) +
                         np.mean([t["price"] for t in window_troughs])) / 2
            norm_upper = upper_slope / avg_price * 100 if avg_price else 0
            norm_lower = lower_slope / avg_price * 100 if avg_price else 0

            # Broadening: upper slopes UP, lower slopes DOWN (diverging)
            if norm_upper <= 0.02 or norm_lower >= -0.02:
                continue

            # Width should be increasing
            width_start = (upper_slope * bar_start + upper_int) - (lower_slope * bar_start + lower_int)
            width_end = (upper_slope * bar_end + upper_int) - (lower_slope * bar_end + lower_int)

            if width_end <= width_start:
                continue

            confidence = round(min(1.0,
                (upper_r2 + lower_r2) / 2 * 0.5 +
                min(total_touches / 8, 0.3) +
                0.2
            ), 2)

            all_pivots = sorted(window_peaks + window_troughs, key=lambda p: p["bar_index"])

            patterns.append({
                "pattern": "broadening_formation",
                "direction": "neutral",
                "start_date": str(all_pivots[0]["date"]),
                "end_date": str(all_pivots[-1]["date"]),
                "pivots": [
                    {"date": str(p["date"]), "price": round(p["price"], 4),
                     "role": p["type"]} for p in all_pivots
                ],
                "upper_slope": round(norm_upper, 4),
                "lower_slope": round(norm_lower, 4),
                "confirmed": False,
                "confidence": confidence,
            })

    return patterns
